...
describe("a library to control the terminal's look using ANSI escape codes", function()
	local look = require '.'
	...
	describe("can change the display of a sentence", function()
		spec("to many different builtin types", function()
			local yay = look.underline + look.bold + look.italic + "yay " + look.invert + "inverted :D" + look.normal
			assert.equals(yay, escape "$[4;1;3myay$[7minverted :D$[m")
		end)

		spec("to many different colors", function()
			local yay = look.red + "yay " + look.italic + look.yellow + "with display attributes too!"
			assert.equals(yay, escape "$[31myay $[3;93mwith display attributes too!")
		end)
	end)
end)
